...
namespace TheArtOfDev.HtmlRenderer.Core.Utils
{
	internal static class HtmlUtils
	{
		...
		static HtmlUtils()
		{
			_list = new List<string>(new string[13]
			{
				"area", "base", "basefont", "br", "col", "frame", "hr", "img", "input", "isindex",
				"link", "meta", "param"
			});
			_encodeDecode = new KeyValuePair<string, string>[4]
			{
			...
			_decodeOnly = new Dictionary<string, char>(StringComparer.InvariantCultureIgnoreCase);
			_decodeOnly["nbsp"] = ' ';
			_decodeOnly["rdquo"] = '"';
			_decodeOnly["lsquo"] = '\'';
			_decodeOnly["apos"] = '\'';
		...
		public static string DecodeHtml(string str)
		{
			if (!string.IsNullOrEmpty(str))
			{
				...
				KeyValuePair<string, string>[] encodeDecode = _encodeDecode;
				for (int i = 0; i < encodeDecode.Length; i++)
				{
					KeyValuePair<string, string> keyValuePair = encodeDecode[i];
		...
		public static string EncodeHtml(string str)
		{
			if (!string.IsNullOrEmpty(str))
			{
				for (int num = _encodeDecode.Length - 1; num >= 0; num--)
				{
					str = str.Replace(_encodeDecode[num].Value, _encodeDecode[num].Key);
				}
			}
			return str;
		}
		...
		{
			for (int num = str.IndexOf("&#", StringComparison.OrdinalIgnoreCase); num > -1; num = str.IndexOf("&#", num + 1))
			{
				bool flag = str.Length > num + 3 && char.ToLower(str[num + 2]) == 'x';
				int num2 = num + 2 + (flag ? 1 : 0);
				long num3 = 0L;
				while (num2 < str.Length && CommonUtils.IsDigit(str[num2], flag))
				{
					num3 = num3 * (flag ? 16 : 10) + CommonUtils.ToDigit(str[num2++], flag);
				}
				num2 += ((num2 < str.Length && str[num2] == ';') ? 1 : 0);
		...
		{
			for (int num = str.IndexOf('&'); num > -1; num = str.IndexOf('&', num + 1))
			{
				int num2 = str.IndexOf(';', num);
				if (num2 > -1 && num2 - num < 8)
				{
					string key = str.Substring(num + 1, num2 - num - 1);
					char value;
					if (_decodeOnly.TryGetValue(key, out value))
					{
						str = str.Remove(num, num2 - num + 1);
						str = str.Insert(num, value.ToString());
					}
				}
			}
			return str;
