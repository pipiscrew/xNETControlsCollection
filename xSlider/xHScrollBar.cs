...
namespace Utilities.xSlider
{
	...
	[ToolboxItem(false)]
	[Browsable(false)]
	[Description("Provides enhanced horizontal content-scrolling capabilities and extended customization options at design time.")]
	[DebuggerStepThrough]
	[EditorBrowsable(EditorBrowsableState.Never)]
	[Designer(typeof(xDesigner))]
	public class xHScrollBar : UserControl
	{
		...
		protected enum ScrollBarVisualStyles
		{
			ScrollBar,
			Slider
		}
		...
		public class xDesigner : ControlDesigner
		{
			...
			public override SelectionRules SelectionRules
			{
				get
				{
					xHScrollBar HScrollBar = (xHScrollBar)base.Control;
			...
			public override DesignerActionListCollection ActionLists
			{
				get
				{
					if (actionLists == null)
						...
						{
							new xHScrollBarActionList(base.Component)
						};
					return actionLists;
				}
			}
		...
		public class xHScrollBarActionList : DesignerActionList
		{
			private xHScrollBar xControl;
			...
			public xHScrollBarActionList(IComponent component)
				: base(component)
			{
				xControl = component as xHScrollBar;
		...
		private xHSlider.ThumbSizes _thumbSize = xHSlider.ThumbSizes.Medium;
		...
		[Category("x Properties")]
		[Description("Sets the border-thickness of the ScrollBar.")]
		...
		public virtual int ThumbLength
		{
			...
			set
			{
				if ((value >= _minimumThumbLength) & (value < base.ClientRectangle.Width))
				{
					_thumbLength = value;
					_thumb.Width = value;
				}
				else if (value < _minimumThumbLength)
				{
					...
					_thumb.Width = _minimumThumbLength;
				}
		...
		[Category("x Properties")]
		[Browsable(false)]
		[Description("Sets the duration the ScrollBar will wait before it shrinks back when inactive.")]
		...
		public virtual int ShrinkSizeLimit
		{
			...
			set
			{
				try
				{
					if (value < 0 || value > base.Height)
						throw new ArgumentOutOfRangeException("The shrink-size limit is outside the appropriate range, that is, between 0 and the ScrollBar's Height.");
		...
		public virtual xHSlider.ThumbSizes ThumbSize
		{
			get
			{
				return _thumbSize;
		...
		protected override void OnMouseMove(MouseEventArgs e)
		{
			...
			if (base.Capture && e.Button == MouseButtons.Left && !_thumbIsMovingInTimer)
			{
				if (AllowCursorChanges && IsMouseInThumb())
					Cursor = Cursors.Hand;
				SetScrollBounds();
				_thumb.Left = e.X - _mouseX;
				if (_thumb.Right >= _bottomScrollbarHeightBound)
					_thumb.Left = _bottomScrollbarHeightBound - _thumb.Width + (_thicknessBound - 1);
				if (_thumb.Left <= _topScrollbarHeightBound)
					_thumb.Left = _topScrollbarHeightBound;
				_value = ConvertRange(_topScrollbarHeightBound, _bottomScrollbarHeightBound - _thumb.Width, Minimum, Maximum, _thumb.Left);
				_thumbIsMoving = true;
				if (this.Scroll != null)
					this.Scroll(this, new ScrollEventArgs(_value));
				if (this.ValueChanged != null)
		...
		protected override void OnMouseClick(MouseEventArgs e)
		{
			...
			if (e.Button == MouseButtons.Left && !_allowMouseCaptures)
			{
				SetScrollBounds();
				_thumb.Left = e.X - _mouseX;
				if (_thumb.Width >= _bottomScrollbarHeightBound)
					_thumb.Left = _bottomScrollbarHeightBound - _thumb.Width + (_thicknessBound - 1);
				if (_thumb.Left <= _topScrollbarHeightBound)
					_thumb.Left = _topScrollbarHeightBound;
				if (!IsPointInRect(Control.MousePosition, _thumb.DisplayRectangle) && !_thumbIsMoving && !AllowIncrementalClickMoves)
				{
					_clickedPosition = ConvertRange(_topScrollbarHeightBound, _bottomScrollbarHeightBound - _thumb.Width + 1, Minimum, Maximum, e.X - (_thumbLength / 2 - _thicknessBound * 2));
		...
		private void DrawScrollBar()
		{
			try
			{
				...
				if (VisualStyle == ScrollBarVisualStyles.Slider)
				{
					...
					rectangle = new Rectangle(DisplayRectangle.X + num / 2, DisplayRectangle.Height / 2 - num2, base.Width - num, num2);
				}
				...
				DrawThumb(ref graphics, new Pen(new SolidBrush(ThumbColor), num), rectangle, num, ThumbLength);
				MoveThumb(_value);
				graphics.Dispose();
			}
			catch (Exception)
		...
		{
			if (thickness * 2 + 2 > base.Height)
				thickness = 1;
			Bitmap image = null;
			try
			{
				image = new Bitmap(_thumb.Width, base.Height - (thickness * 2 + 2));
			}
			catch (Exception)
			{
			}
			if (_thumbStyle == ThumbStyles.Inset)
			{
				...
				_thumb.Top = thickness + _thumbMargin;
				_thumb.Height = base.Height - (_thumbMargin + 1) - (thickness * 2 + (_thumbMargin - 1));
			}
			else if (_thumbStyle == ThumbStyles.Proportional)
			{
				_thicknessBound -= _thicknessBound + 1;
				_thumb.Top = thickness - 1;
				_thumb.Height = base.Height - thickness + 1;
			}
			if (_thumb.Height <= 0)
			{
				_thicknessBound = thickness + base.Height / 2 - 1;
				_thumb.Top = thickness + base.Height / 2 - 1;
				_thumb.Height = 1;
			}
			using (graphics = Graphics.FromImage(image))
			{
				if (VisualStyle == ScrollBarVisualStyles.Slider)
				{
					...
					RectangleF rect = new RectangleF(new Point(1, (_thumb.Height - 17) / 2), new Size(16, 16));
					Rectangle rect2 = new Rectangle(new Point(2, (_thumb.Height - 17) / 2), new Size(2, 16));
					if (_thumbSize == xHSlider.ThumbSizes.Large)
					{
						rect = new RectangleF(new Point(1, (_thumb.Height - 17) / 2), new Size(16, 16));
						rect2 = new Rectangle(new Point(2, (_thumb.Height - 17) / 2), new Size(2, 16));
					}
					else if (_thumbSize == xHSlider.ThumbSizes.Medium)
					{
						rect = new RectangleF(new Point(1, (_thumb.Height - 17) / 2), new Size(14, 14));
						rect2 = new Rectangle(new Point(2, (_thumb.Height - 17) / 2), new Size(2, 14));
					}
					else if (_thumbSize == xHSlider.ThumbSizes.Small)
					{
						rect = new RectangleF(new Point(1, (_thumb.Height - 17) / 2 + 1), new Size(12, 12));
						rect2 = new Rectangle(new Point(2, (_thumb.Height - 17) / 2 + 1), new Size(2, 12));
					}
		...
		private void DrawLayout(Graphics graphics, Rectangle Bounds, int radius)
		{
			...
			graphicsPath.AddRectangle(new Rectangle(Bounds.X + 3, Bounds.Y, Bounds.Width - 6, Bounds.Height));
		...
		{
			_clickedPosition = ConvertRange(_topScrollbarHeightBound, _bottomScrollbarHeightBound - _thumb.Width + 1, Minimum, Maximum, PointToClient(Control.MousePosition).X - (_thumbLength / 2 - _thicknessBound * 2));
			if (AllowIncrementalClickMoves)
			{
				if (_clickedPosition > _value)
				{
					...
					{
						if (_thumb.Right + _largeChange > PointToClient(Cursor.Position).X)
						{
							Value = _clickedPosition;
							_thumbDirection = Direction.Right;
						}
						else
						{
							Value += _largeChange;
				...
				{
					if (_thumb.Left - _largeChange >= PointToClient(Cursor.Position).X)
					{
						Value -= _largeChange;
						_thumbDirection = Direction.Left;
					}
					else
					{
						Value = _clickedPosition;
		...
		{
			if (_autoHidden || base.Height == _shrinkSizeLimit)
			{
				_isGrowing = true;
				...
				transition.add(this, "Height", _scrollBarSize);
				transition.run();
			}
		}
		...
		private void Shrink(object state)
		{
			...
			if (!mouseInScrollBar && !scrollOptionsVisible && !_thumbIsMoving)
			{
				...
				transition.add(this, "Height", _shrinkSizeLimit);
				transition.run();
				_timer.Dispose();
			}
		}
		...
		private void MoveThumb(int to, bool allowTransition = true)
		{
			SetScrollBounds();
			_readyToFill = false;
			int num = ConvertRange(Minimum, Maximum, _topScrollbarHeightBound, _bottomScrollbarHeightBound - _thumb.Width + (_thicknessBound - 1), to);
			...
			if (allowTransition)
			{
				if (_allowScrollingAnimations)
				{
					...
					transition.add(_thumb, "Left", num);
					transition.run();
				...
				else
				{
					...
					transition2.add(_thumb, "Left", num);
					transition2.run();
			...
			else
			{
				...
				transition3.add(_thumb, "Left", num);
				transition3.run();
		...
		private void SetScrollBounds()
		{
			if (_thumbStyle == ThumbStyles.Inset)
			{
				_bottomScrollbarHeightBound = base.Width - _thicknessBound * 2 - BorderThickness + 1 + 3;
				_bottomThumbHeightBound = _thumb.Width - _thicknessBound + 1;
				_topScrollbarHeightBound = _thicknessBound + BorderThickness - 1;
			}
			else
			{
				_thicknessBound = 2;
				_bottomScrollbarHeightBound = base.Width - _thicknessBound * 2 - BorderThickness + 4 + 3;
				_bottomThumbHeightBound = _thumb.Width - _thicknessBound + 1;
				_topScrollbarHeightBound = _thicknessBound + BorderThickness - 3 - 1;
			}
		}
		...
		{
			if (applyUniqueCheck)
				return PointToClient(Control.MousePosition).X >= _thumb.Left && PointToClient(Control.MousePosition).X <= _thumb.Right;
		...
		private void InitializeComponent()
		{
			...
			System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(Utilities.xSlider.xHScrollBar));
